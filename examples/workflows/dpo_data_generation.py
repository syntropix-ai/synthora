...
def score_response(
    ...
    history1 = [msg.to_openai_message() for msg in history1[1:]]
    history2 = [msg.to_openai_message() for msg in history2[1:]]
    _history1 = "\n".join([f"{msg['role']}: {msg['content']}" for msg in history1])
    _history2 = "\n".join([f"{msg['role']}: {msg['content']}" for msg in history2])
    ...
    result = {
        "chosen": history1 if resp.score1 > resp.score2 else history2,
        "rejected": history2 if resp.score1 > resp.score2 else history1,
