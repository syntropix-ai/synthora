...
class BasePrompt(str):
    """A base class for handling prompt templates that extends the built-in str class.

    This class provides functionality for template string formatting with argument validation.
    Inherits from str to maintain string-like behavior while adding prompt-specific features.
    """
    ...
    def args(self) -> Set[str]:
        ...
        Returns:
            Set[str]: A set of unique argument names found in the prompt template,
    ...
    def format(self, **kwargs: Any) -> "BasePrompt":  # type: ignore[override]
        ...
        Returns:
            BasePrompt: A new BasePrompt instance with the template arguments replaced.
                       Returns self if no template arguments exist.
        """
        if not self.args:
            return self
        return BasePrompt(
    ...
    def __get_pydantic_core_schema__(
        ...
        Returns:
            StringSchema: A string schema for Pydantic serialization
        """
