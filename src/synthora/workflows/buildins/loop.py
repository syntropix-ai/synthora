...
def for_loop(task_name_or_offset: str, num: int, counter_name: Optional[str] = None) -> BaseTask:
    counter_name = counter_name or f"{task_name_or_offset}_counter_{num}" + "".join(
    ...
    def _loop(context: BaseContext, *args: Any, **kwargs: Any) -> Any:
        ...
        if isinstance(task_name_or_offset, int):
            ...
            workflow = context.workflow.get_workflow_by_task(task_name_or_offset)
            if not workflow:
...
def while_loop(task_name_or_offset: str, condition_func: Callable[..., bool]) -> BaseTask:
    @task(flat_result=True)
    def _loop(context: BaseContext, *args: Any, **kwargs: Any) -> Any:
