...
from synthora.workflows import task, BaseTask, BaseScheduler, ThreadPoolScheduler, ProcessPoolScheduler, AsyncTask
...
class TestSchedulers:
    ...
    def test_parallel_workflow(self):
        flow = (BaseTask(add).si(1, 2) | BaseTask(add).si(3, 4)).set_flat_result(True) >> BaseTask(add)
        assert flow.run() == 10

        flow = (ThreadPoolScheduler() | BaseTask(add).si(1, 2) | BaseTask(add).si(3, 4)).set_flat_result(True) >> BaseTask(add)
        assert flow.run() == 10

        flow = (ProcessPoolScheduler() | BaseTask(add).si(1, 2) | BaseTask(add).si(3, 4)).set_flat_result(True) >> BaseTask(add)
        assert flow.run() == 10
    
    def test_nested_workflow(self):
        flow1 = BaseScheduler() | BaseTask(add).si(1, 1) | BaseTask(add).si(1, 2)
        ...
        flow2 = ThreadPoolScheduler() >> BaseTask(add).s(1) >> flow1 >> BaseTask(add) | BaseTask(add).si(1, 2)
        assert isinstance(flow2, ThreadPoolScheduler)
        assert flow2.run(1) == [5, 3]

        flow2 = ProcessPoolScheduler() >> BaseTask(add).s(1) >> flow1 >> BaseTask(add) | BaseTask(add).si(1, 2)
    ...
    def test_workflow_immutable(self):
        flow = (BaseTask(add).si(1, 2) >> BaseTask(add).s(3)).set_immutable(True)
    ...
    def test_workflow_flat_result(self):
        flow = BaseScheduler() | BaseTask(add).si(1, 2) | BaseTask(add).si(1, 2)
    ...
    def test_workflow_starmap(self):
        ...
        flow = ProcessPoolScheduler.starmap(test_task, [(1, 2), (3, 4), (5, 6)])
        assert flow.run() == [3, 7, 11]
